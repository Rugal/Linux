...
public abstract class HibernateBaseDao<T, ID extends Serializable> extends HibernateSimpleDao {
	...
	private static final Logger LOG = Logger.getLogger(HibernateBaseDao.class.getName());
	...
	private void updaterCopyToPersistentObject(Updater<T> updater, T po,
		...
		for (String propName : propNames) {
			...
			try {
				...
				LOG.info("property complete!");
			} catch (Exception e) {
				throw new RuntimeException(
					MessageFormat.format("copy property to persistent object failed: '{0}'", propName), e);
			}
		}
	}
